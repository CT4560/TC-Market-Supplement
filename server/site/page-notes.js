...
export async function notesPage({ config }) {
  ...
  const element = h(
    ...
    h("h2", { text: "資料從哪裡來" }),
    h("p", { text: "繁中服有一批舊染劑與色素，Universalis 沒有它們的價格資料。這裡的資料來自玩家使用 Market Board Collector 外掛：玩家在市場板查看這些物品時，外掛會把看到的掛單與成交紀錄匿名回報到這裡。沒有任何程式會主動去抓資料，也沒有人被要求去查價。" }),
    h("p", { text: "外掛只讀取市場板傳來的封包，不會操作遊戲。回報的內容只有市場板上本來就公開顯示的資料：世界、物品、掛單的單價／數量／雇員名稱，以及成交紀錄的單價／數量／買家名稱／成交時間。" }),
